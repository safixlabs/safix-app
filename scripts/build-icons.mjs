...
const source = join(root, "assets", "brand", "safix-app-logo.png")
const CARBON = "#070c0c"

const mark = `data:image/png;base64,${readFileSync(source).toString("base64")}`

// Use the supplied 1800px brand artwork for every size, never an already-reduced icon.
const TARGETS = [
  { path: ["public", "logo.png"], size: 512, inset: 0, transparent: true },
  { path: ["app", "icon.png"], size: 256, inset: 0, transparent: true },
  { path: ["app", "apple-icon.png"], size: 180, inset: 0 },
  { path: ["public", "icons", "icon-192.png"], size: 192, inset: 0 },
  { path: ["public", "icons", "icon-256.png"], size: 256, inset: 0 }
]

const card = (size, inset, transparent = false) => ({
  type: "div",
  props: {
    style: {
      display: "flex",
      ...
      width: "100%",
      height: "100%",
      background: transparent ? "transparent" : CARBON
    },
    children: {
      type: "img",
      props: {
...
async function main() {
  for (const target of TARGETS) {
    const response = new ImageResponse(card(target.size, target.inset, target.transparent), {
      width: target.size,
      height: target.size
    })
