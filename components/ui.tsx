  ...
  return (
    <div className="flex flex-col gap-2">
      <div
        ...
        className="h-1.5 w-full overflow-hidden rounded-[var(--corner-small)] bg-line"
      >
...
export function Segmented<T extends string>({
  ...
  return (
    <div role="group" aria-label={label} className="flex rounded-[var(--corner-control)] border border-control p-1">
      {options.map(option => (
        <button
          key={option.value}
          onClick={() => onChange(option.value)}
          aria-pressed={value === option.value}
          className={`flex-1 rounded-[var(--corner-small)] px-4 py-2 text-[13px] font-medium tracking-[-0.01em] transition-colors ${
    ...
    <div role="group" aria-label={`Quick amounts for ${label}`} className="flex gap-2">
      {[0.25, 0.5, 0.75, 1].map(fraction => (
        <button
          ...
          className="terminal-hover-surface flex-1 rounded-[var(--corner-control)] border border-control py-1.5 text-[12px] tracking-[-0.01em] text-mist transition-colors hover:border-mint hover:text-mint disabled:cursor-not-allowed disabled:border-line disabled:text-haze"
        >
...
export function UsdgTag() {
  return (
    <span className="pointer-events-none absolute right-3 top-1/2 flex -translate-y-1/2 items-center gap-1.5 rounded-[var(--corner-control)] border border-line bg-panel px-2 py-1 text-[11.5px] tracking-[-0.01em] text-mist">
      <UsdgMark />
      USDG
    </span>
  )
...
export function Badge({ label }: { label: string }) {
  const network = /testnet|mainnet|live onchain/i.test(label)
  return (
    <span className={`${network ? "robinhood-badge " : ""}inline-flex items-center gap-2 rounded-[var(--corner-control)] border border-line bg-panel/80 px-3.5 py-1.5 text-[12px] tracking-[-0.02em] text-haze`}>
      <span className={`h-1.5 w-1.5 rounded-full ${network ? "bg-current" : "bg-mint"}`} />
      {label}
    </span>
  )
}
...
export function NotDeployed({ chainName }: { chainName: string }) {
  return (
    <section className="terminal-empty flex flex-col gap-3 rounded-[var(--corner-panel)] border border-line bg-panel p-5 sm:p-7">
      <h2 className="text-[15px] font-semibold tracking-[-0.01em] text-fog">Nothing is deployed on <RobinhoodText>{chainName}</RobinhoodText></h2>
...
export function PageHeader({ title, lead, badge }: { title: string; lead: string; badge?: string }) {
  return (
    <header className="terminal-page-header flex flex-col gap-4">
      <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-3">
        <h1 className="text-[30px] font-bold leading-[1.1] tracking-[-0.035em] text-fog sm:text-[36px] md:text-[42px]">
          {title}
        </h1>
        {badge ? <Badge label={badge} /> : null}
      </div>
      <p className="max-w-[620px] text-[14px] leading-[1.75] tracking-[-0.01em] text-mist md:text-[15px]">
        <RobinhoodText>{lead}</RobinhoodText>
      </p>
    </header>
  )
}
...
export function Stat({ label, value, hint }: { label: ReactNode; value: string; hint?: string }) {
  return (
    <div className="terminal-stat rounded-[var(--corner-panel)] border border-line bg-panel/80 p-5 sm:p-6">
      <p className="flex items-center gap-1.5 text-[12px] font-medium tracking-[0.015em] text-haze">{label}</p>
...
export function Panel({ title, children }: { title: ReactNode; children: ReactNode }) {
  return (
    <section className="terminal-panel rounded-[var(--corner-panel)] bg-panel/80 p-5 sm:p-6">
      <h2 className="terminal-panel-heading flex items-center gap-2 text-[16px] font-semibold leading-snug tracking-[-0.015em] text-fog">{title}</h2>
...
export function Reason({
  ...
  return (
    ...
    >
      {typeof children === "string" ? <RobinhoodText>{children}</RobinhoodText> : children}
    </p>
  )
}
...
export function PrimaryButton(props: ButtonHTMLAttributes<HTMLButtonElement>) {
  ...
  return (
    <button
      {...rest}
      className={`rounded-[var(--corner-control)] bg-mint px-6 py-3 text-[14px] font-semibold tracking-[-0.01em] text-ink transition-colors hover:bg-mint-bright disabled:cursor-not-allowed disabled:bg-line disabled:text-haze ${className ?? ""}`}
    />
  )
}
...
export function GhostButton({
  ...
  return (
    <button
      {...rest}
      className={`terminal-hover-surface rounded-[var(--corner-control)] border border-control ${sizing} font-medium tracking-[-0.01em] text-mist transition-colors hover:border-mint hover:text-mint disabled:cursor-not-allowed disabled:border-line disabled:text-haze disabled:hover:border-line disabled:hover:text-haze ${className ?? ""}`}
    />
  )
}
...
export function Field(props: InputHTMLAttributes<HTMLInputElement>) {
  ...
  return (
    <input
      ...
      className={`w-full rounded-[var(--corner-control)] border border-control bg-carbon/60 px-4 py-3.5 text-[15px] tracking-[-0.01em] text-fog transition-colors placeholder:text-haze focus:border-mint ${className ?? ""}`}
    />
  )
}
...
export function HealthBadge({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 whitespace-nowrap rounded-[var(--corner-control)] border border-line px-2.5 py-1 text-[12px] tracking-[-0.01em] ${copy.tone}`}
      title={copy.blurb}
    >
  ...
  return (
    <div className="flex items-center gap-3">
      <div
        ...
        className="relative h-1.5 w-full max-w-[140px] overflow-hidden rounded-full bg-line"
      >
